...
默认将 **Google Drive `/Family Health Data`** 视为项目根目录。
...
如果 `gog` 能完成任务，先用 `gog`，不要发明额外流程。
如果 `gog` 缺少所需能力、报错或权限不足，则改用当前环境中可用的文件/表格工具执行等价动作；如果仍无法安全写入，只汇报已完成部分、卡点和待确认项，不做猜测性写回。

不要删除源文件，除非用户明确要求。
不要把此 skill 用作医疗诊断、治疗建议或急症分诊工具。
...
不要：
- 在 `/Family Health Data` 外操作，除非用户明确改范围
- 删除原始资料
- 用不确定信息覆盖已确认事实
- 把档案整理扩展成医学诊断结论
- 在低置信度情况下自动合并重复记录
...
- 2026-03-17：精简 description；增加 trigger examples；增加新建成员流程；增加 error handling；去除跨文件重复内容
