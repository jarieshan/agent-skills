...
首次调用时，检查当前skill所在目录下的 `config.json` 是否存在。
...
3. 在该路径下创建初始结构：
   - `INBOX.md`（含 `# Ideas`、`# Tasks`、`# Done` 三个空 section）
   - `PROJECTS.md`（空文件）
   - `projects/` 目录
...
- 一级列表 `- [状态] 描述` 表示事项
- 子列表用于补充说明，格式自由
- 完成事项移到 `# Done` section 下，去掉状态标记，仅保留描述文本
- INBOX 用 `# Ideas`、`# Tasks`、`# Done` 三个 section 组织
- 项目文件用 `# Notes`、`# Tasks`、`# Done` 三个 section 组织
...
- "xxx 已经完成了" — 找到对应任务，从 `# Tasks` 移到 `# Done`（去掉状态标记）
...
- **同一件事的不同阶段**（如"历史补录"→"转为例行化"）应合并为**一个任务**，阶段作为子项记录，而非拆成多个独立任务
- 判断标准：如果多个条目完成的是同一个目标、交付同一个成果，就应该是一个任务
- 子项用于记录阶段、步骤或进展，不需要状态标记
